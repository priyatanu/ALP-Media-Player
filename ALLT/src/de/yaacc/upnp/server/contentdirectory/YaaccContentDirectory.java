...
import de.yaacc.player.MultiContentPlayer;
...
public class YaaccContentDirectory   {
	...
	public static String target = "false";

	@UpnpStateVariable(defaultValue = "0")
	public static String status = "false";

	@UpnpStateVariable(defaultValue = "0", sendEvents = false)
	public static String targetnew = "false";
	...
	public YaaccContentDirectory(Context context) {
		...
		//MultiContentPlayer.setSRTListener(this);
		try {
			db.createDataBase();
			...
			looper = handlerThread.getLooper();
			 mHandler = new Handler(looper) {
				public void handleMessage(Message msg) {
					Log.d("ZAA", "handleMessage " + msg.what + " in " + Thread.currentThread() + msg.obj);
                    String s = ""+ msg.arg1;
					setTarget(msg.obj.toString(),s);
				};
			};
	...
	@UpnpAction
	public void setTarget(@UpnpInputArgument(name = "NewTargetValue") String newTargetValue, @UpnpInputArgument(name = "NewTarget") String NewTarget) {

		String targetOldValue = target;
		target = newTargetValue;
		targetnew = NewTarget;
		String statusOldValue = status;
		status = newTargetValue + "|" +NewTarget ;
		String statusOldVal = value;
		value = NewTarget;
		Log.d("Hola", "setTarget" + value);
		...
		getPropertyChangeSupport().firePropertyChange("targetnew", statusOldValue, targetnew);
		// This will send a UPnP event, it's the name of a state variable that triggers events
		getPropertyChangeSupport().firePropertyChange("Status", statusOldValue, status);
		getPropertyChangeSupport().firePropertyChange("value", statusOldValue, value);
	...
	public BrowseResult browse(
		...
		Log.d("BZA", "UPNP Server" + "YaaccContentDirectoryBrowseResult");
		...
		try {
			Log.d("ZAA", "UPNP setTarget" + "YaaccContentDirectoryBrowseResult ronnet changed status" + status);
            ListenerThread r = new ListenerThread();
	...
	public BrowseResult browse(String objectID, BrowseFlag browseFlag,
		...
		Log.d("BZA", "UPNP Server" + "YaaccContentDirectoryBrowseResult browse"  );
		Log.d("BZA", "Browse: objectId: " + objectID
		...
		if (false) {
            ...
            Log.d("BZA", "UPNP Server" + "YaaccContentDirectoryBrowseResult browseFlag" +browseFlag  );
			if (browseFlag == BrowseFlag.METADATA) {
	...
	private ContentBrowser findBrowserFor(String objectID) {
		Log.d("BZA", "ContentBrowser" + "findBrowserFor"  );
