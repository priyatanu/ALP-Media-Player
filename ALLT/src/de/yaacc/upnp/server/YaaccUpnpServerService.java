...
public class YaaccUpnpServerService extends Service {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see android.app.Service#onStart(android.chii2ontent.Intent, int)
	 */
	@Override
	...
	private void showNotification() {
		...
		NotificationCompat.Builder mBuilder = new NotificationCompat.Builder(this).setOngoing(true).setSmallIcon(R.drawable.ic_launcher)
				.setContentTitle("ALLT Upnp Server")
	...
	private LocalDevice createMediaRendererDevice() {
		...
		try {
			...
			// part of ModelDet and version number
					new DeviceDetails("ALLT - MediaRenderer (" + getLocalServerName() + ")",
	...
	private LocalDevice createMediaServerDevice() {
		// Used from our friend chii2/src/
		LocalDevice device;
		String versionName;
		...
		try {
			...
			DeviceDetails yaaccDetails = new DeviceDetails(
                    "ALLT - MediaServer(" + getLocalServerName() + ")", new ManufacturerDetails("ALLT",
					"ALLT"), new ModelDetails(getLocalServerName() + "-MediaServer", " Android UPnP AV MediaServer",
					versionName));
	...
	private String getLocalServerName() {
		return preferences.getString(getApplicationContext().getString(R.string.settings_local_server_name_key), "ALLT");
	}
	...
	private LocalService<YaaccContentDirectory> createContentDirectoryService() {
		...
		contentDirectoryService.setManager(new DefaultServiceManager<YaaccContentDirectory>(contentDirectoryService, null) {
			...
			protected YaaccContentDirectory createServiceInstance() throws Exception {
				Log.d("ZAA", "Upnp Service YaaccContentDirectory" );
	...
	private LocalService<SrtDiscovery> createSRTService() {
		/*LocalService<SrtDiscovery> srtDiscovery = new AnnotationLocalServiceBinder().read(SrtDiscovery.class);
		Log.d("ZAA", "Upnp Service createSRTService" );
		srtDiscovery.setManager(new DefaultServiceManager<SrtDiscovery>(srtDiscovery, null) {
			...
			@Override
			protected SrtDiscovery createServiceInstance() throws Exception {
				Log.d("ZAA", "Upnp Service createServiceInstance" );
				return new SrtDiscovery(getApplicationContext());
			}
		});*/
		return null;
	}
