...
public class BackgroundMusicService extends Service {
    ...
    public void play() {
        ...
        if (player != null && !player.isPlaying()) {
            MediaPlayer.TrackInfo t[] = player.getTrackInfo();
            for (int i=0;i< t.length;i++) {

            }
            Log.d("BZA", "Music Service Background before start videos");
            player.start();
    ...
    public void setMusicUri(Uri uri) {
        Log.d("BZA", "changing datasource uri to:" + uri.toString());
        if (player != null) {
            player.release();
        }
    ...
    public int getCurrentPosition() {
        ...
        if (player != null) {
            try {
                ...
                Log.d("BZA", "currentPosition"+ currentPosition);
            } catch (Exception ex) {
