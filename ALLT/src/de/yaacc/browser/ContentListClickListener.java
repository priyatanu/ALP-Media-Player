...
public class ContentListClickListener implements OnItemClickListener {
    ...
    public void onItemClick(AdapterView<?> listView, View arg1, int position,
                            long id) {
        Log.d("BZA", "ContentListClickListener onItemClick");
    ...
    private void playAll() {
        Log.d("BZA", "ContentListClickListener playAll");
        ...
        if (result == null || (result.getResult() != null && result.getResult().getItems().size() == 0)) {
            ...
            Log.d("BZA", "Browse result of parent no direct play all" + currentObject.getTitle());
            if(result.getResult() != null && result.getResult().getContainers().size() > 0){
                ...
                Log.d("BZA", "Browse result else part play alls" + currentObject.getTitle());
                play(upnpClient.initializePlayers(currentObject));
            }
        } else {
            ...
            Log.d("BZA", "Another else part Browse result of parent no direct play all" + currentObject.getTitle());
    ...
    public boolean onContextItemSelected(DIDLObject selectedDIDLObject, MenuItem item, Context applicationContext) {
        Log.e("BZA", "onContextItemSelected" +selectedDIDLObject.getTitle() );
    ...
    private void play(List<Player> players) {
        for (Player player : players) {
            if (player != null) {
                Log.e("BZA", "Content List Click Listener play");
                player.play();
                break;
            }
        }
